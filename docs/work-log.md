...
- **Topic**: Conversations tab — show a logical conversation as one coherent multi-turn thread instead of many fragmented "n+1 message" cards.
- **Summary**: A single request already carries the entire conversation history, but the old thread view stitched one user+assistant block per HTTP request from `prompt_text`/`response_text`, so a 19-message request rendered as System + last Tool Result + Assistant. Re-engineered end to end: (1) compute a `conversation_key` at enqueue — client conversation-id header → `cid:{ip}:{id}` (prioritized, env-configurable header list), else head-key `hk:{ip}:{model}:{sha16(system + first user message)}` which survives the tool-result reformatting that broke fingerprint chaining; (2) `GET /proxy/conversation_thread` groups a conversation, splits into segments at compaction boundaries (message-count drops), picks each segment's canonical (max `message_count`, tie-break latest) and returns only canonical `messages[]` + that turn's assistant response — dedupes growing prefixes to one instance; (3) subscribe-on-open `/live` model streams chunk content only to connections subscribed to the open conversation's `conversation_key`, lightweight lifecycle events still broadcast to all, snapshot sent on subscribe, no content on connect; (4) frontend renders full `messages[]` per segment with "context compacted" dividers, subscribes on open / unsubscribes on close, no auto-open; (5) `request_body` stored untruncated by default (`MAX_REQUEST_BODY_BYTES` cap) so ~1M-token histories render intact. Removed fingerprint chaining computation/lookup and the `conversation_diagnostics` endpoint + Chain Diagnostics UI (superseding v4.14).
- **Key Findings**: The fragmentation had two root causes — fragile fingerprint-chained `session_id` (broke on client divergence/summarization) and never rendering a request's full `messages[]`. Head-key grouping accepts that a summarization which rewrites the system/first-user head starts a new conversation (only one new conversation per summarization). The `conversation_diagnostics_removed` test was dropped because the proxy's catch-all forwards unknown `/proxy/*` paths upstream (so a removed route doesn't cleanly 404). Auto-migration adds 3 nullable columns + `ix_conversation_key`; dormant v4.14 fingerprint columns left in place (SQLite drops are destructive).
- **Related Files**: `src/database.py`, `src/data_access.py`, `src/smart_proxy.py`, `src/proxy_endpoints.py`, `src/live_broadcaster.py`, `scripts/migrate_db.py`, `static/dashboard/app.js`, `static/dashboard/index.html`, `static/dashboard/app.css`, `tests/test_conversation_grouping.py`, `tests/test_admin_dashboard.py`, `tests/test_database.py`, `docs/changelog/v4.15_CONVERSATION_VIEW_REENGINEER.md`, `docs/TODO.md`

### v4.14 Conversation chaining diagnostics
- **Topic**: Conversations — determine why a single logical conversation (e.g. a GitHub Copilot agent run) splits into multiple sessions.
- **Summary**: Reviewed the content-based session chaining: on request arrival the proxy hashes `messages[:-1]` into an incoming fingerprint and looks for a prior same-IP request whose stored outgoing fingerprint (`[messages + response]`) matches; match reuses the session, else a new unique session is created. Only the outgoing fingerprint was persisted, so post-hoc it was impossible to tell a proxy miss from a genuinely divergent client history. Added observability (no matching-algorithm change): persist `incoming_conversation_fingerprint`, `session_matched_request_id`, `prefix_message_count`; a `GET /proxy/conversation_diagnostics` admin endpoint that classifies each break as `proxy_miss` (matching prior fingerprint existed but wasn't linked) or `client_divergence` (prefix matches no prior response, with the first divergent message shown); a `session_chain_decision` debug log; and a Conversations-tab "Chain diagnostics" panel.
- **Key Findings**: Chaining requires request N+1's normalized prefix to byte-equal request N's normalized `[messages + response]`; any inserted/trimmed/rewritten history (common in agentic Copilot flows) breaks it. The divergence recompute reuses the exact live normalization helpers (`_normalize_for_fingerprint`, `_normalize_tool_calls_for_fingerprint`, `_extract_text_from_content`) via lazy import to stay faithful to production, and marks detail unavailable when `request_body` was truncated at 256KB.
- **Related Files**: `src/database.py`, `src/data_access.py`, `src/smart_proxy.py`, `src/proxy_endpoints.py`, `scripts/migrate_db.py`, `static/dashboard/*`, `tests/test_database.py`, `tests/test_admin_dashboard.py`, `docs/changelog/v4.14_CONVERSATION_CHAINING_DIAGNOSTICS.md`, `docs/TODO.md`
