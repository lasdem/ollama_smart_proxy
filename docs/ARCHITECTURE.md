...
CREATE TABLE request_logs (
    ...
    request_id VARCHAR(255) UNIQUE NOT NULL, -- Unique request identifier, generated by proxy
    source_ip VARCHAR(45) NOT NULL, -- Client IP address
    model_name VARCHAR(255) NOT NULL, -- Model requested
    prompt_text TEXT, -- Original prompt text
    response_text TEXT, -- Full response text
    timestamp_received TIMESTAMP NOT NULL, -- When request was received
    timestamp_started TIMESTAMP, -- When processing started
    timestamp_completed TIMESTAMP, -- When processing completed
    duration_seconds DECIMAL(10, 3), -- Total time between received and completed
    priority_score INTEGER, -- Calculated priority score at time of processing
    queue_wait_seconds DECIMAL(10, 3), -- Time spent in queue before processing
    processing_time_seconds DECIMAL(10, 3), -- Time spent processing by Ollama
    status VARCHAR(50), -- 'completed', 'failed', 'queued', 'processing'
